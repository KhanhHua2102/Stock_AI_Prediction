...
If you have any crypto holdings in Kraken currently that you don't want the bot to trade, either transfer them out of your Kraken account or be aware the bot will manage them automatically!
...
## Step 5 — Set your folder, coins, and Kraken keys (inside the Hub)
...
- **While you are still in Settings**, click **Kraken API Setup** and do this:

1. Go to [kraken.com](https://kraken.com) and log in.
2. Click your name (top-right) → **Security** → **API**.
3. Click **+ Add key** or **Generate new key**.
4. Give it a name (example: PowerTrader).
5. Set permissions: **Query funds**, **Query open/closed orders**, and **Create & modify orders**.
6. Click **Generate key**. Kraken shows your API Key and Private Key — copy both.
7. Paste both keys into the wizard and click **Save**.
8. Close the wizard and go back to the **Settings** screen.
9. **NOW** click **Save** in Settings.

After saving, you will have two files in your PowerTrader AI folder:  
`kraken_key.txt` and `kraken_secret.txt`  
Keep them private.
